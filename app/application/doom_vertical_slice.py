"""Application orchestration for the approved Doom Eternal AW0.2 slice."""
...
class DoomVerticalSlice:
    ...
    def set_total_playtime_hours(self, total_hours: float) -> None:
    ...
    def add_playtime(self, hours: int, minutes: int) -> None:
        amount = max(0, int(hours)) * 60 + max(0, int(minutes))
        if amount <= 0:
            return
        current = self._current() or self.playthroughs.start(
            self.ref, OperationId.new()
        )
    ...
    def save_personal_rating(self, value: float, review: str = "") -> None:
        current = self._current()
        self.ratings.save_final(
            self.ref,
    ...
    def load_detail(self) -> DoomDetailState:
        ...
        with CatalogUnitOfWork(self.catalog_db) as catalog_uow:
            catalog_item = catalog_uow.catalog.get(DOOM_ETERNAL_ID)
